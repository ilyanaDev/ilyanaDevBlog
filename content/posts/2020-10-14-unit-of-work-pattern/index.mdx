...
![](unit-work-pattern.png)

Like the Repository Pattern, the Unit of Work Pattern is commonly found in data access applications. The Unit of Work provides a means of keeping track of changes to objects from a database (or other data store), so that the application can send the proper information back to the database when the time comes to save those changes.
